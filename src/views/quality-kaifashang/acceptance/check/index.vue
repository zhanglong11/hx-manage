      ...
      <el-form ref="pageQuery" :model="pageQuery" inline>
        <el-form-item label="" prop="equipmentName">
          <el-input
            ...
            style="width: 540px;"
            placeholder="所属分项工程/检验批编号/检验批名称/检验批部位/检验批容量/施工依据/验收依据/创建人"
          />
        </el-form-item>
...
export default {
  ...
  methods: {
    toEdit(id) {
      this.$router.push({ path: 'checkAcceptEdit', params: { id } })
    },
    toDetail(id) {
      this.$router.push({ path: 'checkAcceptDetail', params: { id } })
    },
    remove(rows) {
