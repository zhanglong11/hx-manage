<template>
  <el-dialog width="720px" title="新建任务" :visible="visible" @opened="refresh" @close="close">
    <div id="process-edit">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        actualDuration: [{ required: true, message: '必填' }],
  ...
  watch: {
    'form.parentTaskUniqueId'(val) {
      ...
      if (!this.form.taskUniqueId) {
        const parent = _.find(this.flatList, { taskUniqueId: val })
        const children = _.filter(this.flatList, { parentTaskUniqueId: val })
        const prev = parent ? _.last(children) : _.last(this.taskList)
        if (prev) {
          this.form.planStartTime = moment(prev.planEndTime).add(1, 'days').format('YYYY-MM-DD 00:00:00')
          this.form.planEndTime = moment(prev.planEndTime).add(1, 'days').format('YYYY-MM-DD 23:59:59')
        } else if (parent) {
