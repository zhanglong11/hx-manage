...
export default {
      ...
      tableColumn: [
        ...
        },
        {
          ...
          slots: {
            default: ({ row }) => [
              //执行人
              row.userType.includes(1) && row.auditStatus === 0 ? (
                <el-button type="text" onClick={() => this.toEdit(row.id, 1)}>
                  编辑
                </el-button>
              ) : null,
              row.userType.includes(1) && row.auditStatus === 3 && this.$hasPower('EarlyFileArchiveEditEpc') ? (
                <el-button type="text" onClick={() => this.toEdit(row.id, 1)}>
                  修改
                </el-button>
              ) : null,
              (row.userType.includes(1) && row.auditStatus !== 3) ||
              (row.userType.includes(2) && row.auditStatus !== 1) ? (
                <el-button type="text" onClick={() => this.toEdit(row.id, 3)}>
                  查看
                </el-button>
              ) : null,
              //负责人
              row.userType.includes(2) && row.auditStatus === 1 && this.$hasPower('EarlyFileArchiveAuditEpc') ? (
  ...
  methods: {
    ...
    toEdit(id, status) {
      if (status === 1) {
        this.$router.push({ name: `FileArchiveEditEpc`, params: { id, isEdit: true } })
      } else if (status === 2) {
        this.$router.push({ name: `FileArchiveEditEpc`, params: { id, isAudit: true } })
      } else if (status === 3) {
        this.$router.push({ name: `FileArchiveEditEpc`, params: { id, isDetail: true } })
      }
    },
    async remove(rows) {
