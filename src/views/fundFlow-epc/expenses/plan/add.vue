      ...
      <el-card class="box-card" shadow="never">
        ...
        <div>
          <el-form ref="form" :model="form" :rules="rules" class="form" label-width="150px">
            <el-row>
              <el-col :span="10">
                <el-form-item label="支出计划名称" prop="planName">
          ...
          >
            ...
            <vxe-table-column field="contractName" title="关联合同" :edit-render="{}">
              <template v-slot:edit="{ row }">
                <el-link
                  :type="row.contractName ? 'primary' : 'info'"
                  @click="handleShowContractDialog(row.contractId)"
                  >{{ row.contractName || '请选择关联合同' }}</el-link
                >
              </template>
              <template v-slot="{ row }">
                <el-link type="primary" @click="handleShowContractDialog(row.contractId)">{{
                  row.contractName
                }}</el-link>
              </template>
            </vxe-table-column>
            <vxe-table-column field="planTime" title="预计支出时间" width="200" :edit-render="{}">
              <template v-slot:edit="{ row }">
                <el-date-picker
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      rules: {
        planName: [{ required: true, message: '请输入支出计划名称', trigger: 'blur' }],
      ...
      validRules: {
        ...
        planTime: [{ required: true, message: '请输入预计支出时间' }]
      },
      total: null,
      // 选择合同相关
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.detailList && this.detailList.length === 0) {
        this.$message.warning('请设置支出计划明细')
        return
      }
      ...
      let url = this.id ? `/fund/plan/update` : `/fund/plan/expenditure/add`
      params.id = this.id
