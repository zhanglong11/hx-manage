  ...
  <div class="outContainer">
    <div class="tooltips">
      <el-form ref="filterForm" :model="filterForm" inline>
        <el-form-item label="">
          <el-input v-model="filterForm.keyword" placeholder="合同支出计划名称/创建人" style="width: 280px;">
    ...
    <el-row>
      <el-col>
        <el-button v-if="$hasPower('FundExpensesPlanAddEpc')" type="primary" @click="addNew"
          >新建合同支出计划</el-button
        >
      </el-col>
    </el-row>
    <div style="margin-top: 10px;">
      <Grid ref="table" :filter-form.sync="filterForm" :request="request">
        ...
        <vxe-table-column field="planName" title="合同支出计划名称" min-width="200" />
        ...
        <vxe-table-column title="操作" width="300">
          <template slot-scope="{ row }">
            <el-button v-if="$hasPower('FundExpensesPlanDetailEpc')" type="text" @click="toDetail(row)">查看</el-button>
            <el-button v-if="$hasPower('FundExpensesPlanEditEpc')" type="text" @click="toEdit(row)">编辑</el-button>
            <el-button v-if="$hasPower('FundExpensesPlanPaymentEpc')" type="text" @click="handlePreviewDetail(row)"
              >实际支出明细</el-button
            >
            <el-button v-if="$hasPower('FundExpensesPlanDeleteEpc')" type="text" @click="handleDelete(row.id)"
              >删除</el-button
            >
          </template>
        </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    addNew() {
      this.$router.push({ name: 'FundExpensesPlanAddEpc' })
    },
    toEdit(row) {
      this.$router.push({ name: 'FundExpensesPlanEditEpc', query: { id: row.id } })
    },
    // 查看详情
    toDetail(row) {
      this.$router.push({ name: 'FundExpensesPlanDetailEpc', query: { id: row.id } })
    },
    //删除
    handleDelete(id) {
    ...
    handlePreviewDetail(row) {
      this.$router.push({
        ...
        query: {
          payerId: this.$store.state.user.companyId,
          payYear: row.year,
          payMonth: row.month
        }
      })
