...
@开发人员: 董渊海
@git提交编号: dabb7ea4c11d899ca54838fa1292f23203b6971d
@代码提交修改日期: 2020-03-06
@任务类型: 修改代码，计量器具报废审核 搜索的时间组件添加时分秒，审核和详情的界面样式重新调整
@补加代码注释日期: 2020-04-11
-->
<template>
  <div>
        ...
        <li>
          <el-form-item label="" prop="synthesize">
            <el-input
              ...
              placeholder="编码/名称/规格及型号/报废原因/创建人"
              style="width: 270px;"
          ...
          <el-form-item label="状态" prop="status">
            <el-select v-model="form.status" placeholder="请选择状态" style="width: 140px;">
              <el-option label="报废待审核" value="0"></el-option>
              <el-option label="报废已通过" value="1"></el-option>
              <el-option label="报废已驳回" value="2"></el-option>
            </el-select>
          </el-form-item>
        </li>
    ...
    <vxe-table ref="xTable" border resizable show-overflow :data="tableData" :loading="loading">
      ...
      <vxe-table-column field="status" title="状态">
        <template v-slot="{ row }">
          <span v-if="row.status === 0">报废待审核</span>
          <span v-else-if="row.status === 1">报废已通过</span>
          <span v-else-if="row.status === 2">报废已驳回</span>
        </template>
      </vxe-table-column>
...
<script>
import Api from '@/api/science/audit/measureScrap'
export default {
  ...
  methods: {
    refresh(page, rows) {
      ...
      let scrapStartTime = ''
      let scrapEndTime = ''
      if (this.form.createTime !== null && this.form.scrapTime !== '' && this.form.scrapTime.length !== 1) {
        scrapStartTime = moment(this.form.scrapTime[0]).format('YYYY-MM-DD HH:mm:ss')
        scrapEndTime = moment(this.form.scrapTime[1]).format('YYYY-MM-DD HH:mm:ss')
      }
    ...
    handleDetail(id, row) {
      this.$router.push({ name: 'AuditMeasureScrapDetail', params: { id: id } })
    },
    handleAudit(id, row) {
      this.$router.push({ name: 'AuditMeasureScrap', params: { id: id } })
    },
    handleEdit(id, row) {},
    handleDelete(id, row) {},
