        ...
        <vxe-table-column title="操作" width="180">
          <template slot-scope="{ row }">
            ...
            <template v-if="row.rectifyStatus === 2">
              <el-button
                v-if="$hasPower('FirstInspectRectifyListNoticeEpc') && row.principalId === userId"
                type="text"
            ...
            <template v-if="row.rectifyStatus === 3">
              <el-button
                v-if="$hasPower('FirstInspectRectifyWriteResultEpc') && row.rectifyPrincipal === userId"
                type="text"
            ...
            <template v-if="row.rectifyStatus === 4">
              <el-button
                v-if="($hasPower('FirstInspectDetailWriteCheckEpc') && row.principalId === userId)"
                type="text"
            ...
            <el-button v-if="$hasPower('FirstInspectDetailWriteEpc')" type="text" @click="toDetail(row)"
              >查看</el-button
            >
          </template>
        </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    //填写整改结果
    toWriteRectify(row) {
      this.$router.push({ name: 'FirstInspectRectifyWriteResultEpc', params: { id: row.id } })
    },
    //填写验收结果
    toWriteInspect(row) {
      this.$router.push({ name: 'FirstInspectDetailWriteEpc', params: { id: row.id, isCheck: true } })
    },
    // 查看任务
