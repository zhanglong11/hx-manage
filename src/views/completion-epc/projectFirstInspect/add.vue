              ...
              <el-col :span="8">
                <el-row>
                  <el-form-item label="验收范围" prop="acceptanceScope">
                    <el-input v-model="form.acceptanceScope" placeholder="请输入验收范围" class="inputStyle" />
                  </el-form-item>
                </el-row>
                <el-row>
...
export default {
  ...
  methods: {
    ...
    async submit(isSubmit) {
      ...
      this.id
        ? await Api.update({ ...params, optType: isSubmit ? 2 : 1 })
        : await Api.add({ ...params, optType: isSubmit ? 2 : 1 })
