<template>
  <div class="outContainer">
    ...
    <el-row>
      <el-col>
        <el-button
          v-if="true || $hasPower('AddProjectFirstInspectEpc')"
          icon="el-icon-plus"
          type="primary"
          @click="addNew"
          >新建</el-button
        ...
        <vxe-table-column title="操作" width="200">
          <template slot-scope="{ row }">
            <template v-if="row.acceptanceStatus === 0">
              <el-button v-if="$hasPower('ProjectFirstInspectDetailEpc')" type="text" @click="toDetail(row)"
                >查看</el-button
              >
              <el-button v-if="$hasPower('EditProjectFirstInspectEpc')" type="text" @click="toEdit(row)"
                >编辑</el-button
              >
              <el-button v-if="$hasPower('DeleteProjectFirstInspectEpc')" type="text" @click="handleDelete(row)"
                >删除</el-button
              >
            </template>
            ...
            <template v-else-if="row.acceptanceStatus === 2">
              <el-button
                v-if="$hasPower('FillProjectFirstInspectEpc') && row.principalId === userId"
                type="text"
                @click="toDetail(row, 'inspector')"
                >填报验收结果</el-button
              >
              <el-button v-else type="text" @click="toDetail(row, 'null')">查看</el-button>
            </template>
            <!--已驳回-->
            ...
            <template v-else>
              <el-button
                v-if="$hasPower('ProjectFirstInspectDetailEpc')"
                type="text"
                @click="toDetail(row, 'inspector')"
                >查看</el-button
              >
...
export default {
  ...
  data() {
    return {
      ...
      request: body => {
        return Api.getList({ ...body, projectId: localStorage.getItem('projectId'), type: 1 })
      }
    }
  },
  computed: {},
  ...
  methods: {
    ...
    addNew() {
      this.$router.push({ name: 'AddProjectFirstInspectEpc', params: { id: '' } })
    },
    toEdit(row) {
      this.$router.push({ name: 'EditProjectFirstInspectEpc', params: { id: row.id } })
    },
    // 查看任务
    toDetail(row, type) {
      this.$router.push({
