<template>
  <div class="container">
    ...
    >
      ...
      <vxe-table-column field="itemName" title="索赔项目" :edit-render="{ name: 'input', attrs: { type: 'text' } }" />
      <vxe-table-column
        field="applyMoney"
        title="申请金额"
...
export default {
  ...
  methods: {
    async refresh() {
      let res = await Api.getContractMeasurementPaymentCompensationList({
    ...
    async saveData(row) {
      ...
      if (row.id) {
        res = await Api.updateContractMeasurementPaymentCompensationById(param)
      } else {
        res = await Api.addPaymentCompensation(param)
        row.id = res.data
      }
    },
    cancelRowEvent(row) {
    ...
    async handleDelete(row, rowIndex) {
      if (row.id) {
        await Api.deleteContractMeasurementPaymentCompensationById(row.id)
      } else {
