<template>
  <div class="container">
    ...
    >
      ...
      <vxe-table-column field="itemName" title="迟付款项目" :edit-render="{ name: 'input', attrs: { type: 'text' } }" />
      <vxe-table-column
        field="applyMoney"
        title="申请金额"
...
export default {
  ...
  methods: {
    async refresh() {
      let res = await Api.getContractMeasurementPaymentLateShipmentList({
    ...
    async saveData(row) {
      ...
      if (row.id) {
        res = await Api.updateContractMeasurementPaymentLateShipmentById(param)
      } else {
        res = await Api.addPaymentLateShipment(param)
        row.id = res.data
      }
    },
    cancelRowEvent(row) {
    ...
    async handleDelete(row, rowIndex) {
      if (row.id) {
        await Api.deleteContractMeasurementPaymentLateShipmentById(row.id)
      } else {
