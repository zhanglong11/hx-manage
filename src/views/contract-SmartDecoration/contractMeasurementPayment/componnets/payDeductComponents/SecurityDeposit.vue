<template>
  <div class="container">
    ...
    >
      ...
      <vxe-table-column field="withholdMoney" title="已扣款保证金" />
      <vxe-table-column
        field="applyMoney"
        title="申请金额"
...
export default {
  ...
  methods: {
    async refresh() {
      let res = await Api.getContractMeasurementPaymentCashDepositList({
    ...
    async saveData(row) {
      ...
      if (row.id) {
        res = await Api.updateContractMeasurementPaymentCashDepositById(param)
      } else {
        res = await Api.addPaymentCashDeposit(param)
        row.id = res.data
      }
    },
    cancelRowEvent(row) {
    ...
    async handleDelete(row, rowIndex) {
      if (row.id) {
        await Api.deleteContractMeasurementPaymentCashDepositById(row.id)
      } else {
