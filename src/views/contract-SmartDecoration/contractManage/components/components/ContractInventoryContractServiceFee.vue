  ...
  <div>
    ...
    <div class="tableWrapper">
      <vxe-table ref="xTable" border :data="tableData" auto-resize>
        ...
        <vxe-table-column field="itemWorth" title="项目价值(元)" />
        <vxe-table-column field="serviceContent" title="服务内容" />
        <vxe-table-column field="calculationBasis" title="计算基础" />
        <vxe-table-column field="rate" title="费率(%)"></vxe-table-column>
...
export default {
  name: 'ContractInventoryContractServiceFee',
  props: {
    contractId: {
      type: [Number, String],
      default: ''
  ...
  methods: {
    getData() {
      if (this.contractId) {
        Api.getContractInventoryContractServiceFee({
    ...
    handleExport() {
      Api.exportContractInventoryContractServiceFee({
        contractId: this.contractId,
        categoryCode: this.categoryCode
      }).then(res => {
        /**
    ...
    download(data) {
      ...
      let fileName =
        this.categoryCode === '1' ? '建筑工程_总承包服务费计价表.xls' : '机电设备及安装工程_总承包服务费计价表.xls'
