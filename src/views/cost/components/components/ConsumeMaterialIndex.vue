<template>
  <div class="container">
    ...
    >
      ...
      <vxe-table-column field="quotaCode" title="定额编号" width="150" />
      <vxe-table-column field="quotaName" title="定额名称" width="150" />
      <vxe-table-column title="类别" width="80">耗材</vxe-table-column>
      <vxe-table-column field="categoryCode" title="类别编码" width="150" />
      <vxe-table-column field="categoryName" title="类别名称" width="150" />
      <vxe-table-column field="specificationsModels" title="规格型号" width="150" />
      <vxe-table-column field="unit" title="单位" width="80" />
      <vxe-table-column field="budgetQuantity" title="数量" width="120" />
      <vxe-table-column
  ...
  methods: {
    refresh(isFirstPage = true) {
      ...
      Api.getTabList({
        ...
        this.tableData = res.data.records.filter(item => item.materialType === 1)
      })
    },
    handleAdd() {
