  ...
  <div class="container">
    ...
    >
     ...
     -->
      ...
      />
      <vxe-table-column title="类别" width="80" field="aa">
        <template>
          <!--<span>{{ row.materialType ? (row.materialType === 1 ? '耗材' : '周材') : '材料' }}</span>-->
          <span>材料</span>
        </template>
      </vxe-table-column>
      <vxe-table-column
      ...
      <vxe-table-column
        ...
        :edit-render="{
          ...
          events: { change: changeMaterialType }
        }"
      />
      <vxe-table-column
      ...
      <vxe-table-column
        ...
        :edit-render="{ name: '$input', props: { type: 'number' } }"
      />
      <vxe-table-column field="budgetCombinedPrice" title="合价(元)" width="100">
        <template v-slot="{ row }">
...
export default {
  ...
  methods: {
    ...
    changeMaterialType({ row }, e) {
      row.materialType = e.target.value
    },
    changeNumber({ row }, e) {
