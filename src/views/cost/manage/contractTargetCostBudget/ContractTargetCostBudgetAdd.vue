<template>
   ...
   **/
   -->
    ...
    <el-tabs v-model="activeName" class="topTabs" @tab-click="handleTabClick">
      ...
      <el-tab-pane label="合同项目管理经费测算" name="second">
        <BudgetTreeTable ref="costTreeTable" @tabSave="getTableData"></BudgetTreeTable>
      </el-tab-pane>
    </el-tabs>
...
export default {
  ...
  methods: {
    ...
    async handleSave() {
      await this.$refs.costTreeTable.handleSave()
      this.$router.back()
    },
    async handleSubmit() {
      await this.$refs.costTreeTable.handleSave()
      await Api.addContractTargetCostBudget(this.id).then(res => {
