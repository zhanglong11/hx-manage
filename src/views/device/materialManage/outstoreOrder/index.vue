<template>
  <div>
        ...
        <li>
          <el-form-item label="" prop="content">
            <el-input
              ...
              placeholder="出库单编号/采购单编号/库管员/创建人"
              style="width: 270px;"
              suffix-icon="el-icon-search"
            />
          </el-form-item>
          <el-form-item label="出库日期" prop="checkTime">
            <el-date-picker
    ...
    <vxe-table ref="xTable" border resizable show-overflow :data="tableData">
      ...
      <vxe-table-column field="outStockCode" title="出库单编号"></vxe-table-column>
      <vxe-table-column field="receiveBillCode" title="领料单编号"></vxe-table-column>
      <vxe-table-column field="inspectorName" title="库管员"></vxe-table-column>
      <vxe-table-column field="leaveTime" title="出库日期"></vxe-table-column>
      ...
      <vxe-table-column field="flag" title="是否出库">
        <template v-slot="{ row }">
          {{ !!row.flag ? '是' : '否' }}
        </template>
      </vxe-table-column>
      <vxe-table-column title="操作" width="200">
        <template v-slot="{ row }">
          ...
          <template v-if="row.flag === 0"
            ...
            <a @click="handleOutstore(row.id, row)">出库</a>
          </template>
        </template>
      </vxe-table-column>
    </vxe-table>
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push('AddOutstoreOrder')
    },
    handleDetial(id, row) {
      this.$router.push({
    ...
    handleEdit(id, row) {
      this.$router.push({ name: 'OutstoreOrderEdit', params: { id: id, isEdit: true } })
    },
    async handleDelete(id, row) {
      await this.$confirm('此操作将删除该出库单, 是否继续?', '提示', {
    ...
    handleOutstore(id, row) {
      Api.handleOutstore([id]).then(res => {
        if (res.code === 200) {
