<template>
  <div>
        ...
        <li>
          <el-form-item label="" prop="content">
            <el-input
              ...
              placeholder="耗料清单编号/领料单编号/清点人/创建人"
              style="width: 280px;"
              suffix-icon="el-icon-search"
            />
          </el-form-item>
          <el-form-item label="清点日期" prop="checkTime">
            <el-date-picker
    ...
    <vxe-table ref="xTable" v-loading="loading" border resizable show-overflow :data="tableData">
      <vxe-table-column type="seq" title="序号" width="50"></vxe-table-column>
      <vxe-table-column field="consumeInventoryCode" title="耗料清单编号"></vxe-table-column>
      <vxe-table-column field="receiveBillCode" title="领料单编号"></vxe-table-column>
      <vxe-table-column field="inspectorName" title="清点人"></vxe-table-column>
      <vxe-table-column field="inspectTime" title="清点日期"></vxe-table-column>
  ...
  methods: {
    refresh(isFirstPage) {
      ...
      Api.getList({
        ...
        inspectTimeBegin: this.form.checkTime[0],
        inspectTimeEnd: this.form.checkTime[1],
    ...
    handleAdd() {
      this.$router.push('AddConsumeCheck')
    },
    handleDetial(id, row) {
      this.$router.push({
    ...
    handleEdit(id, row) {
      this.$router.push({ name: 'ConsumeCheckEdit', params: { id: id, isEdit: true } })
    },
    async handleDelete(id, row) {
      await this.$confirm('此操作将删除该耗料清单, 是否继续?', '提示', {
    ...
    handleInstore(id, row) {
      Api.comsumeInstore([row.id]).then(res => {
        if (res.code === 200) {
