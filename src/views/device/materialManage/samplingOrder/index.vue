  ...
  <div>
    <el-form ref="form" :inline="true" :model="form">
      <ul class="search_bar">
         ...
         **/
          -->
          <el-form-item label="取样日期" prop="checkTime">
            <el-date-picker
          ...
          <!--
       /**
        任务名称：BUG#1370 物资与设备管理-材料管理-取样单-缺少一列【创建人】，不需要【状态】
        开发人员：崔洛宜
        日期：2020-03-30
        BUG类型：测试人员发现的研发自测未通过引起的BUG
        **/
    ...
    <vxe-table ref="xTable" border resizable show-overflow :data="tableData">
      ...
      <vxe-table-column field="samplingCode" title="取样单编号"></vxe-table-column>
      <vxe-table-column field="inspectorName" title="取样人员"></vxe-table-column>
      <!--
        /**
         任务名称：BUG#1369 物资与设备管理-材料管理-取样单-日期和时间的调整，见下图
         开发人员：崔洛宜
         ...
         **/
      -->
      <vxe-table-column field="inspectTime" title="取样时间">
        <template v-slot="{ row }">
          {{ moment(row.inspectTime).format('YYYY-MM-DD') }}
        </template>
      </vxe-table-column>
      <vxe-table-column field="remark" title="备注"></vxe-table-column>
      <!--
        /**
         任务名称：BUG#1409 物资与设备管理-材料管理-取样单-列表页缺少一列【创建人】
         开发人员：崔洛宜
         日期：2020-03-30
         BUG类型：测试人员发现的研发自测未通过引起的BUG
         **/
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push('AddSamplingOrder')
    },
    handleDetial(id, row) {
      this.$router.push({
    ...
    handleEdit(id, row) {
      this.$router.push({ name: 'SamplingOrderEdit', params: { id: id, isEdit: true } })
    },
    async handleDelete(id, row) {
      await this.$confirm('此操作将删除该取样单, 是否继续?', '提示', {
