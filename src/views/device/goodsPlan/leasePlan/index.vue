...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({ name: 'LeasePlanAdd' })
    },
    handleDetail(id, row) {
      this.$router.push({ name: 'LeasePlanDetail', params: { id } })
    },
    handleEdit(id, row) {
      this.$router.push({ name: 'LeasePlanEdit', params: { id: id } })
    },
    handleDelete(id, row) {
      this.$confirm(`确定移除 ${row.name} 记录吗？`).then(res => {
        Api.deleteLeasePlan(id).then(res => {
