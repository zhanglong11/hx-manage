...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({ name: 'NeedPlanAdd' })
    },
    handleDetail(id, row) {
      this.$router.push({ name: 'NeedPlanDetail', params: { id: id } })
    },
    handleEdit(id, row) {
      this.$router.push({ name: 'NeedPlanEdit', params: { id: id } })
    },
    handleDelete(id, row) {
      this.$confirm(`确定移除 ${row.name} 记录吗？`).then(res => {
        Api.deleteNeedPlan(id).then(res => {
