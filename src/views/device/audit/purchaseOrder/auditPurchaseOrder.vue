...
@开发人员: 董渊海
@git提交编号: fef55799e90dea322262ec2021f2b092e737ad25
@代码提交修改日期: 2020-03-05
@任务类型: 修改代码，完成 采购单 审核列表接口对接，审核和详情接口对接，审核和详情页样式重新调整
@补加代码注释日期: 2020-04-11
-->
<template>
  <div class="right-panel">
    ...
    <div class="detail-b">
      <div class="detail-b-head">采购明细</div>
...
export default {
  ...
  mounted() {
    ...
    if (id) {
      Api.getPurchaseOrderDetail(id).then(res => {
        let data = res.data
        if (data) {
          this.data = data
        }
  ...
  methods: {
    ...
    handleCancel() {
      this.$router.push({ name: 'PurchaseOrderAudit' })
    },
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          Api.editPurchaseOrder(this.data).then(res => {
