<template>
  <div>
    ...
    <template v-if="$hasPower('MeasurePCDetail')">
      <AddComments :id="id" type="MeasureRecord" :canAdd="$hasPower('MeasurePCAdd')" />
    </template>
    <!-- 批注组件 start -->
    <div class="btn-box">
...
export default {
    ...
    return {
      ...
      tableColumn: [
        ...
        },
        {
          ...
          title: '是否合格',
          slots: {
            default: ({ row }) => [<span>{row.status === 1 ? '合格' : '不合格'}</span>]
          }
        },
        {
          field: 'measurerName',
  ...
  methods: {
    async refresh() {
      let res = await api.getMeasureRecordDetail(this.$route.params.id)
      ...
      let cloneData = res.data.positionList.map(item => {
        return {
          ...
          y: item.yCoordinate,
          id: item.positionNum,
          color: item.positionStatus === 1 ? '#25cc70' : '#e63030'
        }
      })
      this.pointList = cloneData || []
      this.getMeasureDetailList()
      console.log(this.tableList)
    },
    // 实测实量-测点明细列表
    async getMeasureDetailList() {
      let res = await api.getMeasureDetailList({
        measureRecordId: this.$route.params.id,
    ...
    formatterStandard(row) {
      if (
        Math.abs(row.leftValue) <= Math.abs(row.deviationValue) &&
        Math.abs(row.deviationValue) <= Math.abs(row.rightValue)
      ) {
        return '合格'
      } else {
        return '不合格'
