    ...
    <div class="tableWrapper">
      ...
      >
        ...
        <vxe-table-column title="操作">
          <template slot-scope="{ row }">
            <span v-if="row.supplyStatus === 0 && $hasPower('procurementPartyToSupplier')" style="margin: 0 3px;"
              ><a @click="handleDetail(row.id, 1)">通知供应商</a>
            </span>
            <span v-if="$hasPower('procurementPartyDetail')" style="margin: 0 3px;"
              ><a @click="handleDetail(row.id, 2)">查看</a></span
            >
          </template>
        </vxe-table-column>
      </vxe-table>
