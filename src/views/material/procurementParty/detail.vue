<template>
  <div class="container no-padding is-footer">
    <el-card class="box-card">
      <div class="header">
        ...
        <div class="statusWrapper">
          <p class="gray">计划状态</p>
          <p>{{ ['待通知', '采购中', '已完成'][info.supplyStatus] }}</p>
        </div>
      </div>
      <div class="details">
        <el-row>
    ...
    <div class="footer-btn">
      ...
      <el-button v-if="info.supplyStatus === 0 && type === 1" size="lg" type="primary" @click="notifySupplyBtn"
        >通知</el-button
      >
    </div>
  </div>
...
export default {
  ...
  methods: {
    async getPartyDetail() {
      const res = await Api.getPartyDetail(this.id)
      this.info = res.data
      this.tableData = res.data.detailList || []
      this.tableLoading = false
    },
    async notifySupplyBtn() {
      const res = await Api.getNotifySupply(this.id)
      if (res.code === 200) {
        this.$message.success('已发短信通知供应商')
        let that = this
        setTimeout(function () {
          that.goBack()
        }, 1000)
