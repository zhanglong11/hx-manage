  ...
  <div>
    <div class="tooltips">
      <el-form ref="pageQuery" :model="filterForm" inline>
        ...
        <el-form-item>
          <el-button
            v-if="$hasPower('EquipmentRepairAddSmartDecoration')"
            type="primary"
            icon="el-icon-plus"
            @click="toEdit(null)"
            >新建</el-button
...
export default {
    ...
    return {
      ...
      tableColumn: [
        ...
        },
        {
          ...
          slots: {
            default: ({ row }) => [
              row.repairStatus === 1 && this.$hasPower('EquipmentRepairExamineSmartDecoration') ? (
                <el-button type="text" onClick={() => this.showDialog(row.id, 'checkVisible')}>
                  审核
                </el-button>
              ) : null,
              (row.handleState === 0 || row.handleState === 3) &&
              this.$hasPower('EquipmentRepairDetailSmartDecoration') ? (
                <el-button type="text" onClick={() => this.toDetail(row.id)}>
                  执行维修
                </el-button>
              ) : null,
              row.handleState === 1 && this.$hasPower('EquipmentRepairDetailCheckSmartDecoration') ? (
                <el-button type="text" onClick={() => this.toDetail(row.id)}>
                  验收
                </el-button>
              ) : null,
              row.repairStatus === 0 && this.$hasPower('EquipmentRepairEditSmartDecoration') ? (
                <el-button type="text" onClick={() => this.toEdit(row.id)}>
                  编辑
                </el-button>
              ) : null,
              this.$hasPower('EquipmentRepairBasicDetailSmartDecoration') ? (
  ...
  methods: {
    ...
    toEdit(id) {
      _.isNil(id)
        ? this.$router.push({ name: `EquipmentRepairAddSmartDecoration` })
        : this.$router.push({ name: `EquipmentRepairEditSmartDecoration`, params: { id } })
    },
    toDetail(id) {
      this.$router.push({ name: `EquipmentRepairDetailSmartDecoration`, params: { id } })
    },
    toBasicDetail(row) {
      row.handleState
        ? this.$router.push({ name: `EquipmentRepairDetailSmartDecoration`, params: { id: row.id } })
        : this.$router.push({ name: `EquipmentRepairBasicDetailSmartDecoration`, params: { id: row.id } })
    },
