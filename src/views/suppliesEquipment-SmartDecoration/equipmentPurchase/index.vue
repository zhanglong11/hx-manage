  ...
  <div>
    <div class="tooltips">
      <el-form inline>
        ...
        <br />
        <el-button
          v-if="$hasPower('EquipmentPurchaseAddSmartDecoration')"
          class="el-icon-plus"
          type="primary"
    ...
    <Grid ref="table" pageUseQuery :request="request" :filter-form.sync="filterForm">
      ...
      <vxe-table-column title="操作" width="160px">
        <template slot-scope="{ row }">
          <template v-if="row.status === 1 && $hasPower('EquipmentPurchaseExamineSmartDecoration')">
            <span style="margin: 0 3px;"><a @click="handleAudit(row.id)">审核</a></span>
          </template>
          <template v-if="row.status === 2 && $hasPower('EquipmentPurchaseBuySmartDecoration')">
            <span style="margin: 0 3px;"><a @click="handlePurchase(row.id)">采购</a></span>
          </template>
          <template v-if="row.status === 4 && $hasPower('EquipmentPurchaseArrivalSmartDecoration')">
            <span style="margin: 0 3px;"><a @click="handleReceiveConfirm(row.id)">到货确认</a></span>
          </template>
          <template v-if="row.status === 5 && $hasPower('EquipmentPurchaseVerificationSmartDecoration')">
            <span style="margin: 0 3px;"><a @click="handleVerificationAcceptance(row.id)">检定验收</a></span>
          </template>
          <template v-if="row.status === 6 && $hasPower('EquipmentPurchaseWarehousingSmartDecoration')">
            <span style="margin: 0 3px;"><a @click="handleInstore(row.id)">执行入库</a></span>
          </template>

          <span v-if="$hasPower('EquipmentPurchaseDetailSmartDecoration')" style="margin: 0 3px;">
            <a @click="handleDetail(row.id)">查看</a></span
          >
          <template v-if="row.status === 0">
            <span v-if="$hasPower('EquipmentPurchaseEditSmartDecoration')" style="margin: 0 3px;"
              ><a @click="handleEdit(row.id)">编辑</a></span
            >
            <span v-if="$hasPower('EquipmentPurchaseDeleteSmartDecoration')" style="margin: 0 3px;"
...
export default {
  ...
  methods: {
    ...
    addPurchaseItem() {
      this.$router.push({ name: `EquipmentPurchaseAddSmartDecoration` })
    },
    handleDetail(id) {
      this.$router.push({ name: `EquipmentPurchaseDetailSmartDecoration`, params: { id: id } })
    },
    handleEdit(id) {
      this.$router.push({ name: `EquipmentPurchaseEditSmartDecoration`, params: { id: id } })
    },
