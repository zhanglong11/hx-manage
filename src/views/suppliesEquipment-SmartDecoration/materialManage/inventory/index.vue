<template>
  <div class="container">
    ...
    <div class="tooltips">
      <el-form inline>
        ...
        <el-form-item class="flex-wrapper">
          <el-button
            v-if="$hasPower('InventoryAddSmartDecoration')"
            type="primary"
            icon="el-icon-plus"
            @click="addItem()"
            >新建材料</el-button
          >
          <template v-if="$hasPower('InventorySmartDecorationExport')">
            <el-upload
              ref="upload"
              action
    ...
    <!-- 工具栏 end -->
    <el-table :data="dataList" border fit size="small" :row-class-name="tableRowClassName">
      ...
      <el-table-column prop="type" label="材料类型" min-width="120" align="center">
        <template slot-scope="scope">
          {{ scope.row.type === 2 ? '自购' : '甲供' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="100" fixed="right" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="$hasPower('InventoryEditSmartDecoration')"
            type="text"
            size="mini"
          ...
          >
          <el-button
            v-if="$hasPower('InventoryEditSmartDecorationDelete')"
            type="text"
            size="mini"
...
export default {
  ...
  methods: {
    ...
    tableRowClassName({ row }) {
      let { consumeNumber, budgetNumber } = row
      if (consumeNumber > budgetNumber) {
        return 'warning'
      }
    },
    // 选中日期范围时查询数据
