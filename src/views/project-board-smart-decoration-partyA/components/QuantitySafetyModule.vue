                ...
                <p class="name">
                  <b :class="`text-color${i + 1}`">
                    {{ _.get(_.find(qualityLevelList, { value: 4 - i }), 'title') }} （占比：{{
                      ((qualityTableData[item.field] / qualityTotal) * 100) | toFixed2(2)
                    }}
                    %）
                  </b>
                </p>
          ...
          <div class="table-list" style="margin-top: 60px;">
            <ul>
              <li v-for="(item, i) in qualityLevelList" :key="i">
                <p :class="`level text-color${i + 1}`">
                  <b>
                    <!--                    <img :src="require(`../../../assets/images/board/icon${i + 1}.png`)" />-->
                    {{ _.get(_.find(qualityLevelList, { value: 4 - i }), 'label') }}
                  </b>
                </p>
                <p class="name">
                  <b :class="`text-color${i + 1}`">
                    {{ _.get(_.find(qualityLevelList, { value: 4 - i }), 'title') }} （占比：{{
                      ((safeTableData[item.field] / safeTotal) * 100) | toFixed2(2)
                    }}
                    %）
                  </b>
                </p>
...
<script>
import EchartPie from './lib/PieChart'
...
export default {
  ...
  methods: {
    async getSafe() {
      const safeChart = await api.checkCount({ planTypes: [2, 3] })
      const safeData = safeChart.data
      this.securityChartList = [
        { value: safeData.totalResolved, name: '已整改' },
        { value: safeData.totalTODO, name: '未整改' }
      ]
    ...
    // 查询质量问题
    async getQuality() {
      const qualityChartRes = await api.getQualityChartNew()

      const qualityChartData = qualityChartRes.data
      this.qualityChartList = [{ value: qualityChartData.totalUnPass, name: '质量验收不通过' }]

      const qualityQuestionRes = await api.checkCount({ planTypes: [1] })
      const qualityQuestionData = qualityQuestionRes.data

      this.qualityQuestionChartList = [
        { value: qualityQuestionData.totalResolved, name: '已整改' },
        { value: qualityQuestionData.totalTODO, name: '未整改' }
      ]
      console.log(this.qualityQuestionChartList)
...
.chart-pie-con {
  ...
  &:before {
    content: '';
    /*background: url('~@/assets/images/board/chart-line.png');*/
    position: absolute;
    z-index: -1;
    width: 153px;
    height: 153px;
...
.chart-pie-con {
  &:before {
    /*background: url('~@/assets/images/board/chart-pie-line.png');*/
  }
  .title {
    top: auto;
    margin-top: 0;
