...
<template>
  <div>
    ...
    <div class="btn-add">
      <el-button v-if="$hasPower('SafeProcessToggleAdd')" type="primary" icon="el-icon-plus" @click="toEdit(null)"
        >新建</el-button
      >
    </div>
    <div>
      ...
      tableColumn: [
        ...
        },
        {
          ...
          slots: {
            default: ({ row }) => {
              return [
                this.$hasPower('SafeProcessToggleEdit') && row.status === 0 && !this.isAudit ? (
                  <el-button type="text" onClick={() => this.toEdit(row.id, row)}>
                    编辑
                  </el-button>
                ) : null,
                this.$hasPower('SafeProcessToggleDelete') && row.status === 0 && !this.isAudit ? (
                  <el-button type="text" onClick={() => this.remove(row)}>
                    删除
                  </el-button>
                ) : null,
                this.$hasPower('SafeProcessToggleAudit') && row.status === 1 && this.isAudit ? (
                  <el-button type="text" onClick={() => this.toDetail(row.id)}>
                    审核
                  </el-button>
                ) : null,
                (this.$hasPower('SafeProcessToggleDetail') && row.status !== 1) || !this.isAudit ? (
