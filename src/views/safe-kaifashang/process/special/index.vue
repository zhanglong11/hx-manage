...
<template>
  <div>
    ...
    <div class="btn-add">
      <el-button v-if="$hasPower('SafeProcessSpecialAdd')" type="primary" icon="el-icon-plus" @click="toEdit(null)"
        >新建</el-button
      >
    </div>
    <VXTable
      ...
      tableColumn: [
        ...
        },
        {
          ...
          slots: {
            default: ({ row }) => {
              return [
                this.$hasPower('SafeProcessSpecialEdit') && row.status === 0 && !this.isAudit ? (
                  <el-button type="text" onClick={() => this.toEdit(row.id, row)}>
                    编辑
                  </el-button>
                ) : null,
                this.$hasPower('SafeProcessSpecialDelete') && row.status === 0 && !this.isAudit ? (
                  <el-button type="text" onClick={() => this.remove(row)}>
                    删除
                  </el-button>
                ) : null,
                (this.$hasPower('SafeProcessSpecialDetail') && row.status !== 1) || !this.isAudit ? (
