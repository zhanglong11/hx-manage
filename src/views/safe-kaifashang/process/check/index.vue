  ...
  <div>
    <div class="tooltips">
      <el-form inline>
        ...
        <br />
        <el-button
          v-if="$hasPower('SafeProcessCheckAdd')"
          icon="el-icon-plus"
          type="primary"
          @click="$router.push('processCheck/add?planNature=4')"
        >
          安全巡检
        </el-button>
        <el-button
          v-if="$hasPower('SafeProcessCheckAddFixed')"
          icon="el-icon-plus"
          type="primary"
          @click="$router.push('processCheckfixed/add?planNature=5')"
        >
          安全定点巡视
        </el-button>
      </el-form>
    ...
    <div>
      <Grid ref="table" row-id="inspectTaskId" page-use-query :request="request" :filter-form.sync="filterForm">
        ...
        <vxe-table-column field="planName" title="安全计划"></vxe-table-column>
        <vxe-table-column
          field="planNature"
          title="检查性质"
        ...
        ></vxe-table-column>
        <vxe-table-column
          v-if="$hasPower('SafeProcessCheckCommentPCDetail')"
          field="commentStatus"
          title="批注状态"
          width="120px"
        >
        ...
        <vxe-table-column title="操作">
          <template v-slot="{ row }">
            <el-button
              v-if="
                $hasPower('SafeProcessCheckInspect') &&
                row.rectifyStatus === 0 &&
                row.userType === 1 &&
                moment().isSameOrAfter(row.planInspectTime)
              "
              type="text"
              @click="$router.push('processCheck/' + row.inspectTaskId)"
            >
              检查
            </el-button>
            <el-button
              v-if="$hasPower('SafeProcessCheckOrder') && [2, 6].includes(row.rectifyStatus) && row.userType === 1"
              type="text"
            ...
            <el-button
              v-if="$hasPower('SafeProcessCheckRectify') && [3].includes(row.rectifyStatus) && row.userType === 2"
              type="text"
            ...
            <el-button
              v-if="$hasPower('SafeProcessCheckRecheck') && row.rectifyStatus === 4 && row.userType === 1"
              type="text"
            ...
            <el-button
              v-if="$hasPower('SafeProcessCheckDetail')"
              type="text"
              @click="$router.push('processCheck/' + row.inspectTaskId + '/view')"
              >查看</el-button
            >
          </template>
        </vxe-table-column>
