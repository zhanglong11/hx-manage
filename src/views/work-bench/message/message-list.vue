<template>
  <div class="container">
    ...
    <div class="tooltips">
      <el-form inline>
        <el-form-item>
          <el-select v-model="queryForm.type" clearable placeholder="消息类型">
    ...
    <el-table :data="dataList" border stripe size="small">
      ...
      <el-table-column prop="type" label="消息类型">
        <span slot-scope="scope">
          {{ $enum.getDescByValue('MESSAGE_TYPE', scope.row.type) }}
        </span>
      </el-table-column>
...
export default {
  ...
  methods: {
    // 查询列表
    async queryList() {
      const result = await noticeApi.queryListMessage(this.queryForm)
    ...
    toDetails(id) {
      this.$router.push({
        path: `/work-bench/work-bench-message/${id}`
      })
    },
    // 删除消息
    deleteItem(id) {
    ...
    async deleteMessage(id) {
      await workBenchApi.deleteMessage(id)
      this.$message({
        type: 'success',
        message: '删除成功!'
      })
