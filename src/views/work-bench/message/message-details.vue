  ...
  <div class="container no-padding is-footer">
    <el-card class="box-card">
      <div slot="header" class="clearfix details-header">
        ...
        <el-tag>{{ $enum.getDescByValue('MESSAGE_TYPE', details.type) }}</el-tag>
      </div>
      <div class="details">
        <el-row>
          <el-col>
...
export default {
  ...
  methods: {
    ...
    async getDetails() {
      ...
      const result = await workBenchApi.getDetailsMessage(this.id)
    ...
    async getUserInfo(id) {
      const result = await workBenchApi.getUserInfo(id)
      const datas = result.data
