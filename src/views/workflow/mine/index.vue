  ...
  **/
  -->
    <Grid ref="table" auto-search :filter-form.sync="filterForm" :request="request">
      <vxe-table-column field="processInstanceId" title="ID"></vxe-table-column>
      <vxe-table-column field="processInstanceName" title="流程实例名称"></vxe-table-column>
      <vxe-table-column field="processDefinitionName" title="流程定义名称"></vxe-table-column>
      <vxe-table-column field="startUserName" title="申请人"></vxe-table-column>
      <vxe-table-column field="taskStatus" title="流程状态">
        <template slot-scope="scope">
          {{ scope.row.endTime ? '已结束' : '进行中' }}
        </template>
      </vxe-table-column>
      <vxe-table-column field="startTime" title="创建时间"></vxe-table-column>
      <vxe-table-column field="endTime" title="结束时间"></vxe-table-column>
      <vxe-table-column title="操作">
        <template slot-scope="scope">
          <el-button v-if="$hasPower('WorkflowMineView')" size="mini" type="text" @click="viewTask(scope.row, 1)"
            >详情</el-button
          >
        </template>
      </vxe-table-column>
  ...
  data() {
    return {
      ...
      request: body => {
        ...
        return this.axios.workflow.post('activiti/process/instance/started', body).then(res => res.data)
      },
