<template>
  <div class="container">
    ...
    >
      ...
      <vxe-table-column field="itemName" title="费用名称" :edit-render="{ name: 'input', attrs: { type: 'text' } }" />
      <vxe-table-column
        field="applyMoney"
        title="申请金额"
...
export default {
  ...
  methods: {
    async refresh() {
      let res = await Api.getContractMeasurementPaymentOtherWithholdList({
    ...
    async saveData(row) {
      ...
      if (row.id) {
        res = await Api.updateContractMeasurementPaymentOtherWithholdById(param)
      } else {
        res = await Api.addPaymentOtherWithhold(param)
        row.id = res.data
      }
    },
    cancelRowEvent(row) {
    ...
    async handleDelete(row, rowIndex) {
      if (row.id) {
        await Api.deleteContractMeasurementPaymentOtherWithholdById(row.id)
      } else {
