...
export default {
  ...
  methods: {
    async refresh() {
      let res = await Api.getContractMeasurementPaymentOtherPayList({
    ...
    async saveData(row) {
      ...
      if (row.id) {
        res = await Api.updateContractMeasurementPaymentOtherPayById(param)
      } else {
        res = await Api.addPaymentOtherPay(param)
        row.id = res.data
      }
    },
    cancelRowEvent(row) {
    ...
    async handleDelete(row, rowIndex) {
      if (row.id) {
        await Api.deleteContractMeasurementPaymentOtherPayById(row.id)
      } else {
